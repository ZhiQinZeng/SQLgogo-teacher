			 ...
			 <div class="qes_demo" id="qes_deom${item.questionId}">
		      <div id="user" class="user">
		        ...
		        <a href="/SQLgogo/teacher/question.jsp?Qes=${item.questionId}">
		          <img class="img-circle" src="${item.filepath}" style="width:80px; height: 80px;">
		        </a>
		        </div>
...
function me_order(data){
	for(i = 0;i<data.length-1;i++){   //order from big to small
 	   for(j=i+1;j<data.length;j++){
 		   if(data[i].addtime.time<data[j].addtime.time){
 			       let ok = data[i];
 			       data[i] = data[j];
 			       data[j] = ok;
 		   }  
...
function loadData() {
		...
		$.post("../Question?status=new",{page:page},function(data){
...
function skip(){
	  ...
	  if(num>$("#page_count").text()){
	    ...
	    $.post("../Question?status=new",{page:num},function(data){
